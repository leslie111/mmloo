	    ...
	    if(result){
	       	...
	       	if(result.cart_goods_num >0){
	            for (var i = 0; i < result['list'].length; i++){
	            	...
	            	html+='<dl id="cart_item_'+goods['goods_id']+'"><dt class="goods-name"><a href="'+goods['goods_url']+'" target="_blank">'+goods['goods_name']+'</a></dt>';
	            	html+='<dd class="goods-thumb"><a href="'+goods['goods_url']+'" title="'+goods['goods_name']+'" target="_blank"><img src="'+goods['goods_image']+'" target="_blank"></a></dd>';
                ...
                var cart_text = '<span class="ico"></span>购物车 <em>'+result.cart_goods_num+'</em> 件<i class="arrow"></i>'
               obj.find('#incart_text').html(cart_text);
	      } else {
...
function calc_cart_price() {
    //每个店铺商品价格小计
    obj = $('#slide-cart');
    if(obj.children('li').length==0) return;
    //购物车已选择商品的总价格
    var allTotal = 0;
    obj.children('li').each(function(){
        ...
        $(this).find('tr').each(function(){
            if ($(this).find('input[nc_type="eachGoodsCheckBox"]').eq(0).attr('checked') != 'checked') return;
            eachTotal = accAdd(eachTotal,parseFloat($(this).find('.price').children('em').html()));  
        });
        allTotal = accAdd(allTotal,eachTotal);
        $(this).find('span[nc_type="eachStoreTotal"]').eq(0).html(number_format(eachTotal,2));
    });
    $('#cart-total').children('em').html(number_format(allTotal,2));
}

//加载侧边栏购物车信息
function load_slidebar_cart_information(){
	$.getJSON(SITEURL+'/index.php?act=cart&op=ajax_load_slide&callback=?', function(result){
	    var obj = $('#siderCart');
	    if(result.cart_goods_num > 0){
	       	var html = '';
                for(var key in result['list']){
                        html +='<li><div class="store-name"><input class="selectSotre" type="checkbox" nc_type="selectStore"/>' +result['list'][key][0]['store_name']+'<span nc_type="eachStoreTotal">￥<em>'+number_format(0,2)+'</em></span></div><table>';
                        for(var i = 0;i<result['list'][key].length;i++){
                           html +='<tr><td> <input type="checkbox" nc_type="eachGoodsCheckBox" name="cart_id[]" value="'+result['list'][key][i]['cart_id']+'|'+result['list'][key][i]['goods_num']+'"/></td><td class="list-img">';
                           html +='<a href="#"><img src="'+result['list'][key][i]['goods_image']+'" alt=""/></a>';
                           html +='</td><td class="spec">'+result['list'][key][i]['goods_name']+'</td><td class="num">'+result['list'][key][i]['goods_num']+'</td><td class="price">￥<em>'+result['list'][key][i]['goods_price']+'</em></td></tr>';
                       }
                       html +='</table></li>';
                }   
                obj.find('#slide-cart').html(html);
                var total = "<span>已选"+result['cart_goods_num']+"件</span>￥<em>"+number_format(0,2)+"</em>";
                obj.find('#cart-total').html(total);
                
                $('input[nc_type="eachGoodsCheckBox"]').on('click',function(){
                    if (!$(this).attr('checked')) {
                        $('#selectAll').attr('checked',false);
                        $(this).parent().parent().parent().parent().parent().find('input[nc_type="selectStore"]').attr('checked',false);
                    }
                    calc_cart_price();
                });
                $('.selectSotre').click(function(){
                    if ($(this).attr('checked')) {
                        $(this).parent().parent().find('input[nc_type="eachGoodsCheckBox"]').attr('checked',true);
                        $(this).parent().parent().find('input[nc_type="eachGoodsCheckBox"]:disabled').attr('checked',false);
                    } else {
                       $(this).parent().parent().find('input[nc_type="eachGoodsCheckBox"]').attr('checked',false);
                    }
                    calc_cart_price();
                });
               ...
               obj.find(".cart-list-box").css('display','none');
               obj.find(".cart-footer").css("display","none")
               obj.find(".cart-none").css('display','block');
           }
	});
}
...
function drop_topcart_item(cart_id,goods_id){
    $.getJSON(SITEURL+'/index.php?act=cart&op=del&cart_id='+cart_id+'&goods_id='+goods_id+'&callback=?', function(result){
        if(result.state){
            ...
            load_slidebar_cart_information();
        }else{
            alert(result.msg);
        }
    });
...
function homecreditcfc(amount,term){
	var monthly_rate = 0.01;
	if(typeof(amount) == 'string'){
		amount = parseFloat(amount);
	}
	term = parseInt(term);

	//付款金额：2569.22元　=　订单金额：2449.00元　+　分期付款手续费：77.88元（分期手续费以最终收取为准）
    
    if(term == 1){
        var Overpayment = 0.0;
        var str = '付款金额： <font color="#FF0000">'+amount.toFixed(2)+'</font>元　=　订单金额：'+amount.toFixed(2)+'元　+　分期付款手续费：'+Overpayment.toFixed(2)+'元（分期手续费以最终收取为准）';
        var ob =$("#total_homecredit");
        if(ob){ 
            ob.html(str)
        }
        var str1 = amount.toFixed(2)+'元 x '+term+'期（月）';
        var ob1 =$("#monthly_homecredit");
        if(ob1){
            ob1.html(str1)
        }
    }else{
        var Overpayment = 0.0;
        Overpayment = accMul(amount,monthly_rate);
        Overpayment = accMul(Overpayment,term);
        var payment = accAdd(amount,Overpayment);
        var str = '付款金额： <font color="#FF0000">'+payment+'</font>元　=　订单金额：'+amount.toFixed(2)+'元　+　分期付款手续费：'+Overpayment+'元（分期手续费以最终收取为准）';
        var ob =$("#total_homecredit");
        if(ob){ 
            ob.html(str)
        }
        
        var monthly_amount = accDiv(amount,term);
        Overpayment = accMul(amount,monthly_rate);
        monthly_amount = accAdd(monthly_amount,Overpayment);
        
        var str1 = monthly_amount.toFixed(2)+'元 x '+term+'期（月）';
...
function homecreditcfc_goods(amount,term){
	var monthly_rate = 0.01;
	if(typeof(amount) == 'string'){
		amount = parseFloat(amount);
	}
	...
	var Overpayment = 0.0;
    if(term == 1){
        var str = '付款金额： <font color="#FF0000">'+amount.toFixed(2)+'</font>元　=　订单金额：'+amount.toFixed(2)+'元　+　分期付款手续费：'+Overpayment.toFixed(2)+'元';
        var ob =$("#total_homecredit");
    }else{
        Overpayment = accMul(amount,monthly_rate);
        Overpayment = accMul(Overpayment,term);
        var payment = accAdd(amount,Overpayment);
        var str = '付款金额： <font color="#FF0000">'+payment+'</font>元　=　订单金额：'+amount.toFixed(2)+'元　+　分期付款手续费：'+Overpayment+'元';
        var ob =$("#total_homecredit"); 
    }
